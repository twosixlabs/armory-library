...
for batch in dataloader:
    ...
    attacked_linf_norm.update(image, torch.as_tensor(attacked_image))
    ...
    defended_image = normalize(torch.as_tensor(defended_image_unnormed)).cpu().numpy()
    defended_preds = art_classifier.predict(defended_image)
    defended_accuracy.update(
        torch.as_tensor(defended_preds),
        label,
    )
    defended_linf_norm.update(image, torch.as_tensor(defended_image))

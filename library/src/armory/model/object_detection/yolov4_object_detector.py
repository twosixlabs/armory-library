...
from armory.model.object_detection.yolov4_utils import post_processing
...
class YoloV4ObjectDetector(ObjectDetector):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # TODO: (jxc) From YoloV5ObjectDetector, rework for YoloV4ObjectDetector
            ...
            outputs_dict = [
                {
                    "boxes": np.array(
                        [
                            [output[0]*w, output[1]*h, output[2]*w, output[3]*h] for output in outputs[0][:4]
                        ], dtype=np.float32
                    ),
                    "labels": (
                        labels := np.array(
                    ...
                    "scores": np.array(
                        [
                            float(output[4]) for output in outputs[0]
                        ], dtype=np.float32
                    )
                }
                for output in outputs
